...
def login():
    if request.method == 'POST':
        ...
        # Lógica para verificar o login e senha (agora usando hashing de senha)
...
def cadastro_usuario():
    if request.method == 'POST':
        login = request.form['login']
        senha = request.form['senha']
        hashed_senha = generate_password_hash(senha)  # Gera o hash da senha

        # Verifica se o login já existe no banco de dados
        usuario_existente = Usuario.query.filter_by(login=login).first()
        if usuario_existente:
            # Se o login já estiver em uso, exibe uma mensagem de erro
            flash(f'O nome "{login}" já está em uso. Por favor, escolha outro nome de usuario.', 'error')
            return render_template('cadastro_usuario.html')

        # Lógica para cadastrar o usuário no banco (agora com senha criptografada)
